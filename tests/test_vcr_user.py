import pytest
import vcr
from smartabasepy.user_main import get_user, edit_user, create_user, get_group
from smartabasepy.user_option import UserOption, GroupOption
from smartabasepy.user_filter import UserFilter
...
def test_user_df():
    ...
    return DataFrame({
        "first_name": ["Test"],
        "last_name": ["User"],
        "username": ["test.user.2025@example.com"],  # Unique username for testing
...
def test_get_user_vcr(credentials):
    ...
    df = get_user(
        ...
        filter=None,  # Fetch all users to ensure data
        option=option
    )
