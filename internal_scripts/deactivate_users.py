...
from smartabasepy.utils import AMSClient, get_client, AMSError
...
def deactivate_users(
    ...
    """Deactivate users in an AMS instance by setting active to False.

    Uses complete user objects from /api/v2/person/get to preserve existing fields and updates
    the active field via the /api/v2/person/save endpoint. For internal use only.

    Args:
        mapping_df (DataFrame): A DataFrame containing a column with user identifiers
            (e.g., 'username', 'email', 'about', 'uuid').
    ...
    # Step 3: Update each user's active status
    failed_operations = []
    if option.interactive_mode:
        print(f"ℹ Deactivating {len(mapping_df)} users...")
    ...
    for _, row in mapping_df.iterrows():
        ...
        if not user_data:
            ...
            if option.interactive_mode:
                print(f"⚠️ Failed to deactivate user ID {user_id}: User not found")
            continue
        
        # Prepare payload
        user_data = _prepare_user_payload(user_data[0], {"active": False})
    ...
    if failed_df.empty:
        if option.interactive_mode:
            print(f"\n✔ Successfully deactivated all {len(mapping_df)} users.")
    else:
        if option.interactive_mode:
            print(f"\n⚠️ Failed to deactivate {len(failed_df)} users:")
