...
from smartabasepy.utils import get_client, AMSClient, AMSError
...
def update_user_date_of_birth(
    ...
    if "date_of_birth" not in mapping_df.columns:
        raise AMSError("Column 'date_of_birth' not found in mapping_df")
    ...
    # Step 3: Update each user's date of birth
    failed_operations = []
    if option.interactive_mode:
        print(f"ℹ Updating date of birth for {len(mapping_df)} users...")

    for _, row in mapping_df.iterrows():
        user_id = row["user_id"]
        new_dob = row["date_of_birth"]
        ...
        if not user_data:
            ...
            if option.interactive_mode:
                print(f"⚠️ Failed to update date of birth for user ID {user_id}: User not found")
            continue
        
        # Prepare payload
        user_data = _prepare_user_payload(user_data[0], {"dateOfBirth": new_dob})
    ...
    if failed_df.empty:
        if option.interactive_mode:
            print(f"\n✔ Successfully updated date of birth for all {len(mapping_df)} users.")
    else:
        if option.interactive_mode:
            print(f"\n⚠️ Failed to update date of birth for {len(failed_df)} users:")
