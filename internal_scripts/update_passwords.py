...
from smartabasepy.utils import AMSClient, AMSError, get_client
...
def update_user_passwords(
    ...
    if "password" not in mapping_df.columns:
        raise AMSError("Column 'password' not found in mapping_df")
    ...
    failed_operations = []
    if option.interactive_mode:
        print(f"ℹ Updating passwords for {len(mapping_df)} users...")
    ...
    for _, row in mapping_df.iterrows():
        ...
        if not user_data:
            ...
            if option.interactive_mode:
                print(f"⚠️ Failed to update password for user ID {user_id}: User not found")
            continue
        
        # Prepare payload
        user_data = _prepare_user_payload(user_data[0], {"password": new_password})
    ...
    if failed_df.empty:
        if option.interactive_mode:
            print(f"\n✔ Successfully updated passwords for all {len(mapping_df)} users.")
    else:
        if option.interactive_mode:
            print(f"\n⚠️ Failed to update passwords for {len(failed_df)} users:")
