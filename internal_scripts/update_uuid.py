...
def update_user_uuid(
    ...
    if "uuid" not in mapping_df.columns:
        raise AMSError("Column 'uuid' not found in mapping_df")
    ...
    # Step 3: Update each user's UUID
    failed_operations = []
    if option.interactive_mode:
        print(f"ℹ Updating UUID for {len(mapping_df)} users...")
    ...
    for _, row in mapping_df.iterrows():
        ...
        if not user_data:
            ...
            if option.interactive_mode:
                print(f"⚠️ Failed to update UUID for user ID {user_id}: User not found")
            continue
        
        # Prepare payload
        user_data = _prepare_user_payload(user_data[0], {"uuid": new_uuid})
    ...
    if failed_df.empty:
        if option.interactive_mode:
            print(f"\n✔ Successfully updated UUID for all {len(mapping_df)} users.")
    else:
        if option.interactive_mode:
            print(f"\n⚠️ Failed to update UUID for {len(failed_df)} users:")
